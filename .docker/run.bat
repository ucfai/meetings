...
set /p version=<.docker/semester
set container_name="ucfsigai_meeting"

docker run -d --name %@container_name% -v "$(pwd)/.docker/.jupyter:/root/.jupyter" -v "$(pwd)/data:/data" -v "$(pwd):/notebooks" -p "19972:8888" -p "19973:6006" "ucfsigai/%@version%-cpu"